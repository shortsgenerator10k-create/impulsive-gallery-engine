...
import { LayoutGrid, CheckSquare, Square, Trash2, LogIn, Loader2, Plus, Menu, X, BookOpen, Download, Moon, Sun, Palette, Type, Layers } from 'lucide-react';
...
export default function App() {
  ...
  const [themeId, setThemeId] = useState<ThemeId>('ENTERPRISE');
  ...
  if (!user) {
    return (
      <div className={`h-screen w-full flex flex-col md:flex-row overflow-hidden transition-colors duration-700 ${currentTheme.className} ${currentTheme.fontClass} ${isDarkMode ? 'bg-black' : 'bg-white'}`}>
        ...
        <div className={`flex-1 p-12 md:p-24 flex flex-col justify-between relative overflow-hidden transition-all duration-700 ${isDarkMode ? 'bg-[#090D16]' : 'bg-[#F5F5F7]'}`} style={{ background: isDarkMode ? 'radial-gradient(circle at 10% 20%, var(--bg-surface-1) 0%, var(--bg-surface-0) 100%)' : undefined }}>
          <div className="absolute top-0 right-0 w-full h-full opacity-10 pointer-events-none">
             <div className={`absolute top-1/4 right-0 w-96 h-96 blur-[160px] ${isDarkMode ? 'bg-accent/20' : 'bg-indigo-600/10'}`} style={{ backgroundColor: 'var(--accent-glow)' }} />
          </div>
          
          <div className="relative z-10">
            <div className={`flex items-center gap-3 mb-12 group cursor-pointer`} onClick={cycleTheme}>
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${isDarkMode ? 'bg-slate-900 border border-slate-800 text-accent shadow-lg shadow-black/40' : 'bg-black text-white'}`} style={{ color: isDarkMode ? 'var(--accent)' : undefined }}>
                {currentTheme.id === 'ENTERPRISE' ? <Layers size={22} /> : (isDarkMode ? <Moon size={22} /> : <Sun size={22} />)}
              </div>
              <span className={`text-[10px] md:text-xs font-black uppercase tracking-[0.4em] transition-colors ${isDarkMode ? 'text-slate-300 hover:text-white' : 'text-black'}`}>Console Matrix // {currentTheme.name}</span>
            </div>

            <h1 
              className={`text-[12vw] md:text-[10vw] lg:text-[6vw] leading-[0.9] font-black tracking-tighter mb-8 max-w-2xl transition-all ${
                currentTheme.id === 'ENTERPRISE' 
                  ? 'normal-case' 
                  : (currentTheme.id === 'ROYAL' ? 'calligraphy uppercase' : (currentTheme.fontClass === 'font-serif' ? 'italic uppercase' : 'transform -skew-x-12 uppercase engine-glitch'))
            ...
            >
              {currentTheme.id === 'ENTERPRISE' ? (
                <>
                  Organize your <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-violet-500">gallery engine</span>
                </>
              ) : (
                <>
                  Organize <br />
            ...
            <div className="flex flex-col gap-6 max-w-sm">
              <div className="flex items-start gap-4">
                <div className={`p-2 rounded-lg border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-300' : 'bg-black/5 border-black/10 text-black'}`}>
                  <SearchIcon size={16} />
                </div>
                <div>
                   <p className={`font-bold text-sm mb-1 uppercase tracking-widest transition-colors ${isDarkMode ? 'text-slate-200' : 'text-black'}`}>Instant Recovery</p>
                   <p className={`text-xs leading-relaxed font-medium transition-colors ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>Never lose a photo again. Our engine indexes visual context in real-time.</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <div className={`p-2 rounded-lg border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-300' : 'bg-black/5 border-black/10 text-black'}`}>
                ...
                <div>
                   <p className={`font-bold text-sm mb-1 uppercase tracking-widest transition-colors ${isDarkMode ? 'text-slate-200' : 'text-black'}`}>Agentic Tagging</p>
                   <p className={`text-xs leading-relaxed font-medium transition-colors ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>Smart bulk tagging that remembers synonyms and context for you.</p>
                </div>
              </div>
            </div>
          </div>

          <div className={`relative z-10 flex items-center justify-between text-[10px] font-black uppercase tracking-[0.3em] border-t pt-8 transition-colors ${isDarkMode ? 'text-slate-600 border-slate-800' : 'text-black/20 border-black/10'}`}>
        ...
        <div className={`w-full md:w-[450px] p-12 flex flex-col justify-center items-center relative transition-colors duration-700 ${isDarkMode ? 'bg-slate-950 border-l border-slate-900' : 'bg-white'}`}>
          <div className="w-full max-w-xs relative z-10">
            <h2 className={`text-xl font-bold mb-8 tracking-tight transition-colors ${isDarkMode ? 'text-slate-200' : 'text-black'}`}>Console Authorization</h2>
            ...
            <button 
              ...
              className={`w-full flex items-center justify-center gap-4 px-8 py-6 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all shadow-2xl active:scale-95 group ${
                isDarkMode 
                  ? 'bg-accent text-white shadow-accent/20 hover:scale-[1.02] border border-white/10 hover:border-white/20' 
                  : 'bg-black text-white shadow-black/10 hover:bg-orange-600 hover:scale-[1.02]'
              }`}
              style={{ 
                backgroundColor: isDarkMode ? 'var(--accent)' : undefined,
                color: '#ffffff'
              }}
            >
            ...
            <p className={`mt-8 text-center text-[10px] font-bold uppercase tracking-widest leading-loose ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>
  ...
  return (
    ...
    >
      ...
      <main className={`flex-1 flex flex-col min-w-0 transition-colors duration-500 relative ${isDarkMode ? 'bg-black' : 'bg-white'}`}>
        ...
        {/* Header bar */}
        <header className={`h-16 md:h-20 border-b flex items-center justify-between px-4 md:px-8 backdrop-blur-md sticky top-0 z-20 transition-colors ${isDarkMode ? 'bg-[#090D16]/90 border-slate-800' : 'bg-white/90 border-gray-100'}`}>
          <div className="flex items-center gap-3 md:gap-6 min-w-0">
            <button 
              onClick={() => setIsSidebarOpen(true)}
              className={`p-2 -ml-2 md:hidden ${isDarkMode ? 'text-slate-400 hover:text-accent' : 'text-gray-400 hover:text-orange-500'}`}
            ...
            <div className="flex items-center gap-3 min-w-0">
              <div className="flex items-center gap-2 shrink-0">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-white shadow-lg ${isDarkMode ? 'bg-accent/10 text-accent border border-accent/20' : 'bg-black'}`} style={{ color: isDarkMode ? 'var(--accent)' : undefined, borderColor: isDarkMode ? 'var(--accent-glow)' : undefined }}>
                  <Layers size={16} />
                </div>
                <h1 className={`font-extrabold text-sm md:text-base tracking-tight truncate ${isDarkMode ? 'text-slate-100' : 'text-slate-900'}`}>
              ...
              {/* Active Folder Path */}
              <span className={`text-xs font-semibold uppercase tracking-wider hidden sm:inline truncate max-w-[120px] md:max-w-[200px] ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                {currentFolderId ? (folders.find(f => f.id === currentFolderId)?.name || 'Library') : 'Global Root'}
              </span>

              {/* Live Photo Counter Badge */}
              <div className={`hidden md:flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${
                isDarkMode 
                  ? 'bg-slate-950/80 border-slate-800 text-slate-400' 
                  : 'bg-slate-50 border-slate-100 text-slate-600'
              }`}>
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                <span>Live Counter: <strong className={isDarkMode ? 'text-slate-200' : 'text-slate-800'}>{images.length}</strong></span>
                {selectedIds.length > 0 && (
                  <>
                    <span className="text-slate-600">•</span>
                    <span className="text-accent font-semibold" style={{ color: 'var(--accent)' }}>{selectedIds.length} selected</span>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 md:gap-4 shrink-0">
            <div className="relative">
              <button 
                ...
                className={`p-2 rounded-xl border transition-all flex items-center justify-center gap-2 group ${
                  isDarkMode 
                    ? 'bg-slate-900/60 border-slate-800 text-slate-300 hover:border-accent hover:shadow-[0_0_15px_var(--accent-glow)]' 
                    : 'bg-white border-gray-200 text-gray-700 hover:border-black shadow-sm'
                }`}
                title="Change Theme"
              >
                 <Palette size={16} className={`transition-transform group-hover:rotate-12 ${isDarkMode ? 'text-accent' : 'text-orange-500'}`} style={{ color: isDarkMode ? 'var(--accent)' : undefined }} />
                 <span className="text-[10px] font-black uppercase tracking-widest hidden lg:inline">{currentTheme.name}</span>
              </button>
                ...
                {showThemeSelector && (
                  <>
                    ...
                    <motion.div 
                      ...
                      className={`absolute right-0 top-full mt-3 w-64 rounded-2xl border z-50 p-2 shadow-2xl backdrop-blur-3xl transition-all ${
                        isDarkMode ? 'bg-slate-950/95 border-slate-800 shadow-black' : 'bg-white/95 border-gray-100 shadow-gray-200'
                      }`}
                    >
                      <div className="px-3 py-2 mb-2 border-b border-slate-800/50 pb-3">
                        <div className="flex items-center justify-between">
                          <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>System Aesthetics</span>
                      ...
                      <div className="space-y-1">
                        {THEMES.map(t => (
                          <button
                            ...
                            className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all group relative overflow-hidden ${
                              themeId === t.id 
                                ? (isDarkMode ? 'bg-accent text-white font-black' : 'bg-black text-white font-black') 
                                : (isDarkMode ? 'text-slate-400 hover:bg-slate-900 hover:text-white' : 'text-gray-500 hover:bg-gray-50 hover:text-black')
                            }`}
                            style={{ backgroundColor: themeId === t.id && isDarkMode ? 'var(--accent)' : undefined }}
                          >
                            <div className="flex items-center gap-3 relative z-10">
                              <div className={`w-2.5 h-2.5 rounded-full border transition-transform group-hover:scale-125 ${t.isDark ? (isDarkMode ? 'border-white/20' : 'border-black/20') : 'border-gray-300'}`} style={{ backgroundColor: `var(--accent)`, borderColor: themeId === t.id ? 'transparent' : undefined }} />
                            ...
                            {themeId === t.id && (
                              <div className={`w-1.5 h-1.5 rounded-full animate-pulse relative z-10 bg-white`} />
                            )}
                            {/* Hover effect highlight */}
                            <div className={`absolute inset-0 opacity-0 group-hover:opacity-5 transition-opacity bg-white pointer-events-none`} />
                          </button>
                        ))}
                      </div>
                    </motion.div>
            ...
            {selectedIds.length > 0 && (
              <div className="flex items-center gap-2">
                <motion.button 
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl text-[10px] md:text-xs font-black shadow-xl transition-all uppercase tracking-widest ${
                    isDarkMode 
                      ? 'bg-slate-900/60 border border-slate-800 hover:border-accent text-slate-200 hover:shadow-[0_0_15px_var(--accent-glow)]' 
                ...
                <motion.button 
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl text-[10px] md:text-xs font-black shadow-xl transition-all uppercase tracking-widest ${isDarkMode ? 'bg-accent text-white shadow-accent/20 hover:scale-[1.03]' : 'bg-orange-500 text-white shadow-orange-100 hover:bg-orange-600 shadow-sm'}`}
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-3 py-2 border rounded-xl text-[10px] md:text-xs font-black transition-all uppercase tracking-widest hidden lg:flex ${isDarkMode ? 'border-slate-800 text-slate-400 hover:text-accent hover:border-accent/20' : 'border-gray-200 text-gray-400 hover:text-orange-500 hover:border-orange-500/20 shadow-sm'}`}
            >
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-xl text-[10px] md:text-xs font-black shadow-xl transition-all uppercase tracking-widest ${isDarkMode ? 'bg-slate-200 text-slate-950 hover:bg-white' : 'bg-gray-900 text-white shadow-gray-200 hover:bg-black'}`}
            >
              <Plus size={14} /> 
            ...
            {selectedIds.length > 0 ? (
              <div className="flex items-center gap-1">
                <button 
                  onClick={deselectAll}
                  className={`p-2 transition-colors ${isDarkMode ? 'text-slate-400 hover:text-white' : 'text-gray-400 hover:text-gray-900'}`}
                  title="Clear selection"
                >
                  <X size={16} />
                </button>
                <button 
                  className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'text-red-400 hover:bg-red-950/30' : 'text-red-500 hover:bg-red-50'}`}
                  onClick={async () => {
              ...
              <button 
                onClick={selectAll}
                className={`p-2 text-slate-400 hover:text-white md:px-3 md:py-2 md:text-xs md:font-semibold md:flex md:items-center md:gap-2 transition-colors`}
              >
                <CheckSquare size={16} className="md:size-[14px]" />
                <span className="hidden md:inline">Select all</span>
              </button>
            )}

            {/* Profile Dropdown / Sign Out Button at Right End */}
            <div className={`h-6 w-[1px] ${isDarkMode ? 'bg-slate-800' : 'bg-gray-200'}`} />

            <div className="flex items-center gap-3">
              {user.photoURL && <img src={user.photoURL} alt="" className={`w-8 h-8 rounded-full border ${isDarkMode ? 'border-slate-800' : 'border-gray-200'}`} referrerPolicy="no-referrer" />}
              <button id="signout-btn" onClick={() => signOut()} className={`text-[10px] font-black transition-colors uppercase tracking-wider ${isDarkMode ? 'text-slate-400 hover:text-red-400' : 'text-gray-500 hover:text-red-500'}`}>SignOut</button>
            </div>
          </div>
        </header>
          ...
          <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div>
                <div className="flex items-center gap-3 mb-2">
                   <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${isDarkMode ? 'bg-accent text-white shadow-accent/10 border border-accent/20' : 'bg-black text-white'}`} style={{ backgroundColor: isDarkMode ? 'var(--accent)' : undefined }}>
                      <LayoutGrid size={16} />
                   </div>
                   <h2 className={`text-2.5xl font-extrabold tracking-tight uppercase ${themeId === 'ENTERPRISE' ? '' : 'italic transform -skew-x-12'} ${isDarkMode ? 'text-white' : 'text-black'}`}>
                     {currentFolderId ? folders.find(f => f.id === currentFolderId)?.name : 'Global Buffer'}
                   </h2>
                </div>
                <div className={`flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.2em] ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>
                   <span>Inventory: {filteredImages.length} Nodes</span>
                   {searchQuery && (
                     <span className={`px-2 py-0.5 rounded-md animate-pulse ${isDarkMode ? 'text-accent bg-accent/10 border border-accent/20' : 'text-orange-500 bg-orange-50'}`} style={{ color: isDarkMode ? 'var(--accent)' : undefined, backgroundColor: isDarkMode ? 'var(--accent-glow)' : undefined }}>
              ...
              {selectedIds.length > 0 && (
                <div className={`flex items-center gap-2 px-6 py-3 border rounded-2xl shadow-xl transition-all ${isDarkMode ? 'bg-accent/5 border-accent/20 shadow-accent/5' : 'bg-white border-orange-500/20 shadow-orange-100/50'}`} style={{ borderColor: isDarkMode ? 'var(--accent-glow)' : undefined }}>
